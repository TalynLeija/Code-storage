...
#Create data frame
df = pd.DataFrame(Previous_Lotto_numbers,columns=list("ABCDE"))
...
def Lstm_ML(window,columns,train1,label1):
    model = Sequential()
    model.add(Bidirectional(LSTM(52521875,
    ...
    model.add(Bidirectional(LSTM(52521875,
        ...
        return_sequences=True)))
    model.add(Dropout(0.2))#
    model.add(Bidirectional(LSTM(16777216,
        input_shape=(window,columns),
        return_sequences=True)))
    model.add(Bidirectional(LSTM(1048576,
        input_shape=(window,columns),
        return_sequences=True)))
    model.add(Bidirectional(LSTM(16384,
        input_shape=(window,columns),
